...
<div class="bg-white rounded-lg shadow-sm p-6">
    ...
    <form action="{{ route('admin.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" id="event-form" novalidate>
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="judul" value="{{ old('judul', $event->judul) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500 @error('judul') border-red-500 @enderror">
                @error('judul')
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="deskripsi" rows="4" class="w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500 @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $event->deskripsi) }}</textarea>
                @error('deskripsi')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="lokasi" value="{{ old('lokasi', $event->lokasi) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500 @error('lokasi') border-red-500 @enderror">
                @error('lokasi')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @else
                <input type="datetime-local" name="tanggal_waktu" value="{{ old('tanggal_waktu', $event->tanggal_waktu->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500 @error('tanggal_waktu') border-red-500 @enderror">
                @endif
                @error('tanggal_waktu')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div id="tiket-container" class="space-y-4 mb-8">
            ...
            <div class="tiket-row flex gap-4 items-end bg-gray-50 p-4 rounded-lg">
                ...
                <div class="flex-1">
                    ...
                    @else
                    <select name="tikets[{{$index}}][tipe]" class="w-full px-3 py-2 border rounded @error('tikets.' . $index . '.tipe') border-red-500 @enderror">
                ...
                <div class="flex-1">
                    ...
                    @else
                    <input type="number" name="tikets[{{$index}}][harga]" value="{{ (int)$tiket->harga }}" min="0" class="w-full px-3 py-2 border rounded @error('tikets.' . $index . '.harga') border-red-500 @enderror">
                    @error('tikets.' . $index . '.harga')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    @endif
                </div>
                ...
                <div class="w-32">
                    ...
                    @else
                    <input type="number" name="tikets[{{$index}}][stok]" value="{{ $tiket->stok }}" min="1" class="w-full px-3 py-2 border rounded @error('tikets.' . $index . '.stok') border-red-500 @enderror">
                    @error('tikets.' . $index . '.stok')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    @endif
                </div>
        ...
        <div class="mt-6 border rounded-lg p-4 bg-gray-50">
            <h3 class="text-lg font-medium text-gray-900 mb-3">Riwayat Status Event</h3>
            @if($event->eventStatusHistories->isNotEmpty())
            <div class="space-y-2">
                @foreach($event->eventStatusHistories()->orderBy('created_at', 'desc')->get() as $history)
                <div class="flex items-center justify-between bg-white px-4 py-3 rounded border">
                    <span class="text-sm font-medium text-gray-700">{{ $history->status }}</span>
                    <span class="text-xs text-gray-500">{{ $history->created_at->format('d M Y, H:i') }}</span>
                </div>
                @endforeach
            </div>
            @else
            <p class="text-sm text-gray-500">Belum ada riwayat status untuk event ini.</p>
            @endif
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let tiketCount = document.querySelectorAll('.tiket-row').length;
